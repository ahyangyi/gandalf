...
func GetScenegraph(scenegraphMap Map, allowedLayers []int, filter func(string) bool, pointData []types.PointData, sizeData []types.Size) Node {
	if len(scenegraphMap) == 0 && len(sizeData) > 0 && len(pointData) > 0 {
		...
		return Compose(scenegraphMap, scenegraphMap[0], 0, 0, 0, -1, allowedLayers, "", filter, pointData, sizeData)
	}
